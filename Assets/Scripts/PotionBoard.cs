...
public class PotionBoard : MonoBehaviour
{
    ...
    //포션 보드의 2차원 배열 선언
    private Node[,] _potionBoard; // 포션 보드의 2차원 배열 //2차원 배열이란? 2개의 인덱스를 사용하여 데이터를 저장하는 배열입니다. 예를 들어 아파트 생각하면 편합니다. 각 층과 호수를 사용하여 특정 아파트를 지정할 수 있죠. 각 층은 첫 번째 인덱스, 각 호수는 두 번째 인덱스로 생각할 수 있습니다.
    public GameObject potionBoardGo; //포션 보드 게임 오브젝트
    
    //"일단 강의에서 이렇게 만들라 해서 만들긴 하는데 이거 싱글톤 나중에 안쓸거같음"
    public static PotionBoard Instance; //싱글톤 인스턴스 //싱글톤이란? 싱글톤 패턴은 클래스의 인스턴스가 오직 하나만 존재하도록 보장하는 디자인 패턴입니다. 이를 통해 전역적으로 접근 가능한 단일 인스턴스를 제공할 수 있습니다. 보통 Instance라는 정적 변수를 사용하여 구현합니다. "우리 텍스트 RPG만들때 게임메니져 이걸로 만듬"
